...
export async function main(args = process.argv.slice(2)) {
    ...
    if (command === 'init') {
        ...
        await withInitLock(directory, async (checkInterrupted) => {
            ...
            const previous = await readFile(configPath, 'utf8').then(text => JSON.parse(text)).catch((error: any) => { if (error.code !== 'ENOENT')
                throw error; return null; });
            ...
            checkInterrupted();
            try {
                await waitForStack({ address, readBalances, minEth: 3000000000000000n, timeoutMs: positive(flags['timeout-ms'], 180000, '--timeout-ms') });
            }
            catch (error) {
                if (gasError)
                    throw new Error(`${(gasError as Error).message}. Funding not ready; repeat init to resume.`);
                throw error;
            }
            checkInterrupted();
            if (!flags['no-install'])
    ...
    const privateKey = (await readFile(join(directory, '.botkey'), 'utf8')).trim() as Hex;
    if (command === 'gas') {
        const result = await claimGas({ privateKey });
        const receipt = await createBot({}).publicClient.waitForTransactionReceipt({ hash: result.txHash, timeout: 180000 });
    ...
    if (command === 'run' && !flags['dry-run']) {
        const pending = await readFile(pendingPath, 'utf8').catch((error: any) => { if (error.code !== 'ENOENT')
            throw error; return null; });
        if (pending !== null)
    ...
    const interval = positive(flags['interval-ms'], 180000, '--interval-ms');
