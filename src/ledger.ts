import { createPublicClient, createWalletClient, http, parseAbi, parseAbiItem, type Account, type Address, type PublicClient, type WalletClient, } from 'viem';
...
export const parseTokens = (count: number) => BigInt(Math.max(0, Math.round(count))) * 1000000n;
export const parseUSDC = (amount: number) => BigInt(Math.max(0, Math.round(amount * 1e6)));
export function resolveTrade(idx: number, board: {
    clubs?: ({
        name: string;
    } | undefined)[];
}, priced: {
...
export function clients(account?: Account): {
...
export async function sendTrade(wallet: WalletClient, account: Account, trade: {
    positionId: bigint;
    isLong: boolean;
    usdcIn?: bigint;
    minTokensOut?: bigint;
...
export async function chainPrices(client: PublicClient) {
