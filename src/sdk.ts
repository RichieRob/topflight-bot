import { createPublicClient, createWalletClient, encodeFunctionData, fallback, http, isAddress, keccak256, parseUnits, type Address, type Hex, type PublicClient, type TransactionReceipt, } from 'viem';
...
export function quoteBound(quoted: bigint, slippageBps: number, maximum: boolean): bigint {
...
function transport(urls: string[]) {
    ...
    return fallback(urls.map(url => http(url, { timeout: 12000, retryCount: 0 })), {
        rank: false, retryCount: 1, retryDelay: 1000,
    });
}
const zero = '0x0000000000000000000000000000000000000000' as Address;
const normalized = (value: bigint) => Number(value) / 1e6;
export async function readSnapshot(chain: PublicClient, account: Address = zero) {
    ...
    const details = await chain.multicall({
        ...
        contracts: ids.map(id => ({ address: world.ledger, abi: ledgerAbi,
            functionName: 'getPositionDetails', args: [world.marketId, id] } as const)),
    });
    const shares = await chain.multicall({
        blockNumber, allowFailure: false,
        contracts: ids.map(id => ({ address: world.ledger, abi: ledgerAbi,
            functionName: 'ladderPpmOf', args: [id] } as const)),
    });
...
export function createBot(options: BotOptions = {}): Bot {
    ...
    let lastWrite = 0;
    function trade(id: number, value: string | number, isLong: boolean, closing: boolean) {
        const task = queue.then(async () => {
            ...
            let receipt;
            try {
                receipt = await publicClient.waitForTransactionReceipt({ hash, timeout: 120000 });
            }
            catch (cause) {
    ...
    return { address: account?.address, publicClient,
        snapshot: (address?: Address) => readSnapshot(publicClient, address ?? account?.address ?? zero),
        buy: (id: number, usd: string | number) => trade(id, usd, true, false),
        sell: (id: number, tokens: string | number) => trade(id, tokens, false, true),
        fade: (id: number, usd: string | number) => trade(id, usd, false, false),
        cover: (id: number, tokens: string | number) => trade(id, tokens, true, true),
    };
}
